...
public:
  Move(const table_t &table, const board_t &board,
       ...
       const hashsize_t &goal, const int depth, std::queue<mod_t> prev_mods, std::shared_ptr<bool> complete);
  ~Move() {}
};
...
private:
  ...
  BaseMove(const board_t &board, const table_t &table, const hashsize_t &goal,
      ...
      : board(board), table(table), goal(goal), previous_hashes(h),
        Move(table, board, h, goal, depth, {}, std::make_shared<bool>(this->complete)) {}
